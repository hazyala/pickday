...
- Home은 진행 중인 약속, 가능한 날짜, 내 약속 방을 보여주는 메인 허브입니다.
...
| HomeActivity | 필요 | 필요 | 진행 중인 약속, 가능한 날짜, 내 약속 방을 보여주는 메인 화면 |
...
- 향후에는 Repository 계층과 Retrofit API를 연결하여 실제 사용자별 방 생성, 응답 저장, 일정 계산을 서버와 동기화할 예정입니다.
- Google/Kakao 로그인, 푸시 알림, 캘린더 연동은 실제 서비스 확장 단계에서 추가할 기능입니다.
- 화면은 목업을 단순 복제하는 것이 아니라 실제 서비스에서 빈 상태와 데이터 상태가 자연스럽게 보이도록 개선할 예정입니다.
