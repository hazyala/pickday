...
Room Detail 기본 정보는 선택된 `roomId`의 로컬 방 데이터에서 제목, 상태, 마감일/마감 시간, 참여 인원, 응답률, 확정 일정 또는 미정 상태를 표시합니다. 후보 날짜별 가능 인원과 시간대별 선호도 같은 계산값은 별도 계산 단계에서 `ScheduleResult` 기준으로 연결합니다.
